...
void mergeVectors(const vector<vector<int>>& input, vector<int>& result) {
  int num_of_vectors = input.size();
  vector<vector::const_iterator> vec_iters;
  ...
  while (!done) {
    ...
    int all_done = 0;
    for(auto it=vec_iters.cbegin(); it!=it.cend(); it++) {
    ...
    if (all_done == 4) { // When all vec_iters reaches the end
      done = true;
    } else {
